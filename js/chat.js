const form = document.querySelector(".typing-area"),
    ...
    sendCipher = form.querySelector("#sendCipher"),
    sendBlowfish = form.querySelector("#sendBlowfish"),
    chatBox = document.querySelector(".messages-container");
const back = document.querySelector('#back');
let message = "";
decCipher = document.querySelector('#decCipher');
decBlow = document.querySelector('#decBlow');

let decryptAlgorithm = "";
...
sendBtn.onclick = () => {
    ...
    xhr.onload = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                inputField.value = "";
                inputField.placeholder = "Type a message here...";
                scrollToBottom();
            }
        }
    }
...
sendCipher.onclick = () => {
    ...
    if (message == "") {
        inputField.placeholder = "Can't encrypt empty value";
    } else {
        inputField.value = encrypt("ALPHABETA", message.toUpperCase().replaceAll(' ', ''));
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "../php/insert-chat.php", true);
        xhr.onload = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
...
decCipher.onclick = () => {
    decryptAlgorithm = "hill";
    decCipher.classList.add("selected");
    decBlow.classList.remove("selected");
}
decBlow.onclick = () => {
    decryptAlgorithm = "blow";
    decBlow.classList.add("selected");
    decCipher.classList.remove("selected");
}
// Checking if there is a new messages ...
setInterval(() => {
    let xhr = new XMLHttpRequest();
...
function msgSelection() {
    let msg = document.querySelectorAll('.In-message');
    msg.forEach(message => {
        message.onclick = () => {
            if (decryptAlgorithm === "hill") {
                messageDecryptHill(message);
            } else if (decryptAlgorithm === "blow") {
                messageDecryptBlow(message);
            }
        }
    });
}
function messageDecryptHill(message) {
    let msgDecrypted = "";
    let msg_id = document.querySelector('.msg_id');
    let id;
    if (message.textContent.length == 0) {
        inputField.placeholder = "No message Selected";
    } else {
        id = message.lastElementChild.value;
        msg_id.value = id;
        msgDecrypted = message.textContent;
        inputField.value = decrypt("ALPHABETA", msgDecrypted.toUpperCase().replaceAll(' ', '')).toLowerCase();

        let xhr = new XMLHttpRequest();
        xhr.open("POST", "../php/decryption.php", true);
        xhr.onload = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    inputField.value = "";
...
//----------------------------------------------------------------------------------------
// Hill Cipher Algorithm with CBC mode 
function encrypt(key, plaintext) {
    // Getting key array character code 
    let keyArrayCode = findCharCode(key);
    function findCharCode(key) {
        let charCounter = 0;
        let keyArrayCode = [[], [], []];
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                let temp = key.charCodeAt(charCounter);
                temp = 90 - temp;
                temp = 25 - temp;
                keyArrayCode[i][j] = temp;
                charCounter++;
            }
        }
        return keyArrayCode;
    ...
    if (plaintext.length % 3 == 0) {
        ...
        counterText = plaintext.length / 3;
    } else {
        let x = plaintext.length % 3;
        if (x == 2) {
            plaintext = plaintext + "_";
        } else if (x == 1) {
            plaintext = plaintext + "__";
        }
        counterText = plaintext.length;
        counterText = counterText / 3;
    }
    let IV;
    let ciphertext = "";
    ...
    for (let i = 0; i < counterText; i++) {
        let temp = i * 3;
        let plaintextpart = plaintext.slice(temp, temp + 3);
        let ciphertextPart = encryptBlock(plaintextpart, keyArrayCode, IV);
        for (let i = 0; i < 3; i++) {
            IV[i] = ciphertextPart.charCodeAt(i);
        }
        IV = findCharCodeP(IV);
...
function encryptBlock(plaintextPart, keyArrayCode, IV) {

    // Converting every letter in each part to binary code ! 
    let encryptArr = [];

    for (let i = 0; i < 3; i++) {
        encryptArr[i] = plaintextPart.charCodeAt(i);
    }



    //We have three parts of the plaintext so it can match our 3 * 3 key's array

    encryptArr = findCharCodeP(encryptArr);
    // XOR every letter in each part with IV 
    //CBC mode here 
    for (let i = 0; i < 3; i++) {
        encryptArr[i] = (IV[i] ^ encryptArr[i]);
    }
    // Multiply each part of the plaintext with the key array 
    encryptArr = multiplyArrays(keyArrayCode, encryptArr);

    encryptArr = findMod(encryptArr);

    let ciphertext = findCharacterForCode(encryptArr);
...
function decrypt(key, ciphertext) {
    ...
    //[d,e,f]
    //[g,h,i]
    let a = keyArrayCode[0][0];
    let b = keyArrayCode[0][1];
    let c = keyArrayCode[0][2];
    let d = keyArrayCode[1][0];
    let e = keyArrayCode[1][1];
    let f = keyArrayCode[1][2];
    let g = keyArrayCode[2][0];
    let h = keyArrayCode[2][1];
    let i = keyArrayCode[2][2];

    let detK = (a * ((e * i) - (f * h))) - (b * ((d * i) - (f * g))) + (c * ((d * h) - (e * g)));
    ...
    if (detK == 1) {
        ...
        return "Determinant Key might be wrong! ";
    }

    // find the determinant for each value in the matrix 
    let detA = ((e * i) - (f * h));
    let detB = -((d * i) - (f * g));
    let detC = ((d * h) - (e * g));
    let detD = -((b * i) - (h * c));
    ...
    keyArrayCode[0][0] = detA;
    keyArrayCode[0][1] = detB;
    keyArrayCode[0][2] = detC;
    keyArrayCode[1][0] = detD;
    keyArrayCode[1][1] = detE;
    keyArrayCode[1][2] = detF;
    keyArrayCode[2][0] = detG;
    keyArrayCode[2][1] = detH;
    keyArrayCode[2][2] = detI;

    swap(0, 1, 1, 0);
    swap(0, 2, 2, 0);
    swap(1, 2, 2, 1);

    function swap(i, j, sI, sJ) {
        let temp = 0;
        temp = keyArrayCode[i][j];
        keyArrayCode[i][j] = keyArrayCode[sI][sJ];
        keyArrayCode[sI][sJ] = temp;
    }

    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            keyArrayCode[i][j] = keyArrayCode[i][j] * detK;
            keyArrayCode[i][j] = ((keyArrayCode[i][j] % 26) + 26) % 26; // MOD 26 CHOOSE ALWAYS THE POSITIVE VALUE 
        }
    }
    let counterText
    counterText = ciphertext.length / 3;
    let IV;
    let plaintext = "";
    ...
    for (let i = 0; i < counterText; i++) {
        let temp = i * 3;
        let ciphertextpart = ciphertext.slice(temp, temp + 3);
        let plaintextpart = decryptBlock(ciphertextpart, keyArrayCode, IV);
...
function decryptBlock(ciphertextPart, keyArrayCode, IV) {

    // Converting every letter in each part to binary code ! 
    let decryptArr = [];
    let plaintext = "";
    for (let i = 0; i < 3; i++) {
        decryptArr[i] = ciphertextPart.charCodeAt(i);
    }
    // We have three parts of the cipher text so it can match our 3 * 3 key's array

    decryptArr = findCharCodeP(decryptArr);

    // Multiply each part of the cipher text with the key array 
    decryptArr = multiplyArrays(keyArrayCode, decryptArr);

    decryptArr = findMod(decryptArr);

    //XOR every letter in each part with IV 
    for (let i = 0; i < 3; i++) {
        decryptArr[i] = (IV[i] ^ decryptArr[i]);
    }

    plaintext = findCharacterForCode(decryptArr);

    return plaintext;

}
function findCharCodeP(arr) {
    let temp = 0;
    for (let i = 0; i < 3; i++) {
        temp = arr[i];
        temp = 90 - temp;
...
function multiplyArrays(keyArrayCode, arrText) {
    let arr = [];
    let temp = 0;
    let result = 0;
    for (let i = 0; i < 3; i++) {
        // going for the next row => rest the result 
        result = 0;
        for (let j = 0; j < 3; j++) {
            temp = keyArrayCode[i][j] * arrText[j];
            result = temp + result;
        }
        arr[i] = result;
    }
...
function findMod(arr) {
    for (let i = 0; i < 3; i++) {
        arr[i] = ((arr[i] % 26) + 26) % 26;
    }
    return arr;
}
function findCharacterForCode(arr) {
    let temp = 0;
    let finalCipherText = "";
    for (let i = 0; i < 3; i++) {
        temp = arr[i];
...
sendBlowfish.onclick = () => {
    message = inputField.value;
    if (message == "") {
        inputField.placeholder = "Can't encrypt empty value";
    } else {
        inputField.value = crypt('encrypt', message);
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "../php/insert-chat.php", true);
        xhr.onload = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
...
function messageDecryptBlow(message) {
    let msgDecrypted = "";
    let msg_id = document.querySelector('.msg_id');
    let id;
    if (message.textContent.length == 0) {
        ...
        msg_id.value = id;
        msgDecrypted = message.textContent;
        inputField.value = crypt('decrypt', msgDecrypted);
...
function crypt(mode, msg) {
    const key = "ALPHBETA";
    let result = blowfish[mode](msg, key, { outputType: + "Base64", cipherMode: + "CBC" });
